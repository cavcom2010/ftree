...
PRODUCTION_HEALTH_URL="${PRODUCTION_HEALTH_URL:-https://ftree.calvinmazhindu.dev/health/}"
PRODUCTION_BIND="${PRODUCTION_BIND:-127.0.0.1:${PRODUCTION_DEBUG_PORT:-8136}}"
...
Default behavior:
  ...
  - Start Gunicorn in the foreground on 127.0.0.1:8136 for live error visibility.

Options:
  --foreground       Stream Gunicorn logs in this terminal (default).
  --daemon           Start the debug Gunicorn process in the background.
  --bind VALUE       Override debug bind address. Example: 127.0.0.1:8136.
...
Environment:
  PRODUCTION_BIND=127.0.0.1:8136
  PRODUCTION_SERVICE_NAME=ftree-gunicorn.service
  PRODUCTION_HEALTH_URL=https://ftree.calvinmazhindu.dev/health/
...
reload_live_gunicorn() {
  ...
  if ! master_pid="$(live_master_pid)"; then
    echo "Could not find live HeritageTree Gunicorn master process to reload." >&2
    exit 1
  fi
